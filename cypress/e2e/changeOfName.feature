...
Feature: As an internal Hackney User
    ...
    # Scenario Outline: Ineligible person does not have the Change Of Name process available
    #    Given I seeded the database with an asset with a previous tenure
    #    When I visit the processes menu for a person
    #    Then I am on the Processes list page
    #    And I am not able to see the Change Of Name process listed

    @Positive @SmokeTest @wip
    Scenario Outline: Positive Path - Full Change of Name Process
       Given I seeded the database with a person with an active tenure
        ...
        When I select title "<title>" and enter first and last names: "<firstName>" "<lastName>"
        ...
        And Status Stepper is at 'End Case'
        And the person has a new name - "<firstName>" "<lastName>"
