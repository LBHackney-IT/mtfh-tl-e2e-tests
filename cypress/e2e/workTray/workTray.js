...
When("I select one or more of the Patches", () => {
    workTrayPO.patchesSelectAll().click();
});
When("I select oen or more of the Process Status", () => {
...
Then("the existing listing display will REMAIN unchanged", () => {
...
Then("I can see the panel title", () => {
    workTrayPO.filterBy().click();
    workTrayPO.textProcesses().should('exist');
});
Then("a Processes filter in a list with checkbox", () => {
    workTrayPO.checkboxSoleToJoint().should('exist');
});

Then("a Process status filter with a checkbox", () => {
    workTrayPO.processCompleted().should('exist');
    workTrayPO.reqSuppDoc().should('exist');
    workTrayPO.submitForTenureInvest().should('exist');
});
Then("there will be a select all link", () => {
    workTrayPO.filterBy().click();
...
Then("by default the process filter will select ALL processes", () => {
    workTrayPO.filterBy().click();
    workTrayPO.processesSelectAll().should('be.selected');
});
Then("by default the status filter will select ALL statuses", () => {
    workTrayPO.filterBy().click();
    workTrayPO.processStatusSelectAll().should('be.selected');
});
Then("an action button to apply my selected filters", () => {
    workTrayPO.filterBy().click();
    workTrayPO.applyFilters().should('exist');
});
Then("an action button to clear my filter selections", () => {
    workTrayPO.filterBy().click();
    workTrayPO.clearFilters().should('exist');
});
Then("days range of the items being displayed", () => {
    workTrayPO.filterShowDays().should('exist');
});
Then("by default day range is pre-selected as last 30 days", () => {
    workTrayPO.filterShowDays().should('contain', 'Last 30 days');
});
Then("the number of items being displayed", () => {
    workTrayPO.filterShowItems().should('exist');
});
Then("by default number of items displayed is pre-selected as 10 items per page", () => {
    workTrayPO.filterShowItems().should('contain', '10 items');
});
Then("there will be pagination displayed", () => {
    workTrayPO.pagination().should('contain', 'Showing');
});
Then("I can see the Name or Address of the Tenant", () => {
    workTrayPO.personLink().should('exist');
    workTrayPO.propertyLink().should('exist');
});
Then("type of process", () => {
...
Then("state of the process", () => {
    workTrayPO.headerState().should('exist');
});
Then("time remaining for a process", () => {
...
Then("pre-selected to ALL available status", () => {
...
Then("I click 'Apply filters' action button to apply my selected filters", () => {
    workTrayPO.applyFilters().click();
});
...
Then("the info icon is on display next to process status", () => {
...
Then("the link will change to 'Remove All' in a link", () => {
