...
Given("I expect to see NonUkNumber {boolean}", (enabled) => {
    cy.get("input[data-test='phone-number-checkbox']")
        .last()
        .should(enabled ? "be.checked" : "not.be.checked");
});

Given("I check Non-UK Number checkbox with {boolean}", (enabled) => {
...
Given("I add up to 5 contact details of type {string}", (contactType) => {
    for (let i = 0; i < 5; i++) {
        cy.wait(500);
...
Given("I click add a correspondence address", () => {
...
Given("I click save correspondence address", () => {
