...
Given('the person has been added to the tenure', () => {
    addPersonPage.pageAnnouncement().contains('Person added to tenure')
})

Given('the person is added to the tenure page {string} {string} {string}', (title, firstName, middleName) => {
...
Given('the person page is loaded', () => {
    cy.url().should('contain', 'person')
})

Given('I am on the tenure page {string}', (tenureId) => {
    cy.url().should('include', `tenure/${tenureId}`)
})

When('I click the update person button', () => {
...
Then('the person type options are not displayed', () => {
...
Then('the nationality field is not displayed', () => {
    addPersonPage.nationalityContainer().should('not.exist')
})

Then('the national insurance field is not displayed', () => {
...
Then('I click the save equality information button', () => {
...
Then('I select a disability option {string}', (disabilityOption) => {
    addPersonPage
...
Then('I select an ethnicity {string}', (ethnicity) => {
    addPersonPage.ethnicitySelectionBox().select(ethnicity)
})

Then('I select a gender {string}', (gender) => {
    if (gender === 'Male') {
...
Then('I enter {string} into the gender term field', (genderTerm) => {
    addPersonPage.preferredGenderTermField().type(genderTerm)
})

Then('I select a gender identity option {string}', (genderIdentity) => {
    addPersonPage
...
Then('I select a religion or belief {string}', (religionOrBelief) => {
    addPersonPage.religionOrBeliefSelectionBox().select(religionOrBelief)
})

Then('I select a sexual orientation {string}', (sexualOrientation) => {
...
Then('I click save equality information', () => {
    addPersonPage.saveEqualityInformationButton().click()
})
...
Then('the sexual orientation information is not displayed', () => {
...
When("I browse to the 'Add Person to Tenure' page for the tenure", () => {
...
When("I visit the 'Edit person' page for the person", () => {
